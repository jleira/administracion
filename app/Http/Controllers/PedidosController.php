  ...
  public function index()
  {
    ...
    $bodegas=DB::table('app_bodegas')->get();
       ...
       $reservas=DB::table('app_reservas')->where('bodega',Auth::user()->bodega_id)->get(); 
		$bodegas=DB::table('app_bodegas')->where('id',0)->get();
     }
  	
    return View::make("pedidos.buscar")->with(array('reservas'=>$reservas,'bodegas'=>$bodegas));
  }
    ...
    }elseif ($request->generarfactura==4) { 

      $view =  \View::make('pdf.fact')->with(array('reserva'=>$reserva,'factura'=>$factura,'fact'=>1))->render();
      $pdf = \App::make('dompdf.wrapper');
      $pdf->loadHTML($view);
      return $pdf->stream('invoice'); 
         
    }elseif ($request->generarfactura==5) { 

      $view =  \View::make('pdf.fact')->with(array('reserva'=>$reserva,'factura'=>$factura,'fact'=>2))->render();
      $pdf = \App::make('dompdf.wrapper');
      $pdf->loadHTML($view);
      return $pdf->stream('invoice'); 
...
return back();
    ...
    public function crearpedido(Request $request)
    {
      ...
      if ($request->has('cotizar')) {
        ...
        DB::table('app_cotizaciones')->delete();
    ...
    if ($cantidad[$i]> $totalp) {
      ...
      $totalfacturado=valorfactura($productosguardar,$cantidadguardar,$request->cliente);
           $total_facturado=$totalfacturado-$des+$imp; 
               if ($request->has('iva')) {
          $iva=1;
          (float)$total_facturado=$total_facturado*1.19;
        }else{
          $iva=0;
        }

       (float)$totalrestante=$total_facturado-$abono;
...
if ($iva==1) {
  $iva=1;
  (float)$cobrariva=($totalfacturado-$des+$imp)*1.19;
  (float)$total_facturado=$cobrariva;

}else{
  $iva=0;
  (float)$total_facturado=$totalfacturado-$des+$imp;
}
(float)$totalrestante=$total_facturado-$abonototal;
...
$disponible=disponibles($bodega,$fechaini,$fechafin);
     ...
     return View::make("pedidos.rango")->with(array('reservas'=>$disponible));   
 }
}

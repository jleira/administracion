  ...
  <div class="row">
    ...
    @foreach($categorias as $categoria)
      ...
      <div class="box box-primary">
          ...
          <h3 class="box-title">{{validarlista($categoria->valor_lista,4)}} - {{formatofecha($fecha_inicio)}} / {{formatofecha($fecha_fin)}}</h3>
					</div>

          @if($todasbodegas==2)
    @foreach($bodegas as $bodega)
    <div class="box-header">
          <h3 class="box-title">{{validarbodega($bodega)}}</h3>
          <form id="form1" name="form1" role="form" enctype="multipart/form-data" method="POST" action="{{ url('imprimir') }}">
          {{ csrf_field() }}
          <input type="hidden" name="categoria" value="{{$categoria->valor_lista}}">
...
<button type="submit" class="btn btn-success pull-right" name="button" value="4" formtarget="_blank">Descargar(EXCEL)</button>
          </form>
          </div>
            ...
            <div class="box-body">
              ...
              <thead>
                <tr>
                     ...
                     @else
                         <div class="box-header">
          <h3 class="box-title">Todas las bodegas - {{formatofecha($fecha_inicio)}} / {{formatofecha($fecha_fin)}}</h3>
          <form id="form1" name="form1" role="form" enctype="multipart/form-data" method="POST" action="{{ url('imprimir') }}">
          {{ csrf_field() }}
          <input type="hidden" name="categoria" value="{{$categoria->valor_lista}}">
